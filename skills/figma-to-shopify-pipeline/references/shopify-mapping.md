...
7. Check that each generated Liquid component is functionally complete in Shopify, not only visually close.
8. Build a local preview from the same source data whenever practical.
9. Run Playwright against homepage navigation and routed destinations before moving into a real Shopify theme.
10. Use the browser output to compare against the Figma design and refine visual drift with the UI/design helper skills.
11. Reuse a stable unpublished preview theme ID when pushing to Shopify.
12. Re-run Playwright against the Shopify preview and distinguish theme gaps from missing Shopify resources.
