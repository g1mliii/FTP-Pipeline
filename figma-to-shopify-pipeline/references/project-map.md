...
- `scripts/fetch-figma-file.mjs`
  ...
  - For Figma Make files with code resources, start with the app entry and route-bearing files, then read only the specific components/styles needed for normalization.
...
- `scripts/lib/route-specs.mjs`
  ...
  - Theme generation should happen only after the normalized files are locked and re-read as the authoritative source.
...
- `scripts/serve-preview.mjs`
  ...
  - The active design slug must be available in the shell context, typically via `DESIGN_SLUG`, unless the slug is passed directly to the script entrypoint.
...
- Unknown values from the design source should stay explicit in normalization. Do not invent reviews, FAQs, navigation targets, policy links, or backend resources to make the theme look more complete.
- Avoid parallel sub-agent theme generation before normalization is finalized. It tends to create drift between normalized data, schema, asset references, and rendered output.
